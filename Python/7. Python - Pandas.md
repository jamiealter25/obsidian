...
- `pd.read_csv()` has comma separator by default
```python
pd.read_csv("link", sep=",") # by de
```
- **Comma-separated (CSV):** `sep=','` (This is the default, so you can omit `sep` entirely).
    
- **Tab-separated (TSV):** `sep='\t'`
    
- **Pipe-separated:** `sep='|'`
- Data FormatRecommended CodeStandard CSVpd.read_csv("file.csv")Tab-separatedpd.read_csv("file.tsv", sep="\t")Irregular/variable spacespd.read_csv("file.txt", sep=r"\s+")Fixed column widthspd.read_fwf("file.txt")
...
print(df1.iloc[20:31, 0:5])  # prints rows from index 20 to 30 and columns 0 to 4
```
#### Show Data
```python
