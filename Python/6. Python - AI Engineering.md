...
## Comparison Table of Function, Method & Attribute

|     Type      |     Syntax      |   Example   | Key Things                                                |
| :-----------: | :-------------: | :---------: | :-------------------------------------------------------- |
| **Function**  | `name(object)`  |  `len(df)`  | • No dot prefix<br>• Ends with parentheses `()`           |
|  **Method**   | `object.name()` | `df.head()` | • Starts with a dot (`.`)<br>• Ends with parentheses `()` |
| **Attribute** |  `object.name`  | `df.shape`  | • Starts with a dot (`.`)<br>• Has NO parentheses `()`    |

# Load Dataset
- use `!wget` to load dataset directly from web. Here `wget --> web get`.
- use `-O filename.extension` after dataset link. The `-O` option specifies the output filename, saving the downloaded data as `filename.extension` in the current working folder.
```python
# Example loading of Titanic Dataset
!wget https://raw.githubusercontent.com/datasciencedojo/datasets/refs/heads/master/titanic.csv -O titanic.csv
```
